...
export default function PipelineNewPage(): JSX.Element {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createPipeline.mutate({
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <Input
              ...
              disabled={createPipeline.isLoading}
          ...
          <div className="pt-4 flex justify-end gap-3">
            <Button variant="outline" type="button" asChild disabled={createPipeline.isLoading}>
              <Link to="/dashboard">Cancel</Link>
            </Button>
            <Button
              type="submit"
              disabled={createPipeline.isLoading || channelsLoading || !formData.topic || !formData.channel_id}
            >
              {createPipeline.isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {createPipeline.isLoading ? 'Creating...' : 'Start Pipeline'}
            </Button>
          </div>
        </form>
      </div>
