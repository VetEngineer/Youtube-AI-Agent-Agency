...
function ChannelDetailModal({
  ...
  const handleRefresh = () => {
    void refreshMutation.mutate(competitorId)
  }
      ...
      <Card className="bg-card/50 backdrop-blur-sm border-primary/50 relative overflow-hidden">
        ...
        <CardFooter className="border-t border-white/5 pt-3 flex gap-2">
          ...
          <Button
            ...
            onClick={() => void refreshMutation.mutate(competitor.id)}
            disabled={refreshMutation.isLoading}
          >
            {refreshMutation.isLoading ? (
...
export default function CompetitorsPage(): JSX.Element {
  ...
  const addMutation = useAddCompetitor(refetch)
  const deleteMutation = useDeleteCompetitor(refetch)

  const handleAdd = async () => {
    if (!channelIdInput.trim()) return
    setAddError(null)
    try {
      await addMutation.mutate({ youtube_channel_id: channelIdInput.trim() })
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMutation.mutate(deleteCompetitorId)
      setDeleteCompetitorId(null)
      setDeleteError(null)
    } catch {
      setDeleteError('경쟁 채널 삭제에 실패했습니다.')
    }
  }
          ...
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={addMutation.isLoading || !channelIdInput.trim()}
              >
