...
import pytest
from yaa_app.cli import _log_usage_summary
from yaa_core.database.engine import init_db, set_session_factory
from yaa_core.database.models import UsageEventModel
from yaa_core.database.repositories import UsageRepository
from yaa_core.shared.llm_clients import (
    UsageCollector,
...
class TestLogUsageSummary:
    ...
    def test_이벤트_없으면_로그_안남(self, caplog):
        collector = UsageCollector()
        with caplog.at_level(logging.INFO, logger="yaa_app.cli"):
    ...
    def test_실패_있으면_경고_로그(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="yaa_app.cli"):
            _log_usage_summary(collector)
